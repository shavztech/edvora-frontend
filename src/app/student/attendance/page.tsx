  ...
  const loadAttendance = async () => {
    ...
    try {
      ...
      const pastSlots = res.data.filter((booking: Booking) => {
        if (booking.status !== "accepted" || !booking.mentor) return false;
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 sm:p-6 space-y-5">
      ...
      <div className="bg-white p-4 sm:p-5 rounded-[24px] border border-gray-100 shadow-sm">
        <div className="flex items-center gap-2.5 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 mb-4 sm:mb-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="space-y-1.5 col-span-2 sm:col-span-1">
            ...
            <input 
              type="date" 
              className="w-full bg-gray-50 border border-gray-100 px-3 py-2.5 rounded-xl text-[11px] sm:text-xs font-bold focus:bg-white focus:border-indigo-600 focus:ring-4 focus:ring-indigo-600/5 outline-none transition-all"
              value={filterDate}
              onChange={(e) => {
          ...
          <div className="space-y-1.5">
            ...
            <select 
              className="w-full bg-gray-50 border border-gray-100 px-3 sm:px-4 py-3 rounded-xl text-[11px] sm:text-xs font-bold focus:bg-white focus:border-indigo-600 focus:ring-4 focus:ring-indigo-600/5 outline-none transition-all cursor-pointer appearance-none"
              value={filterMonth}
              onChange={(e) => {
          ...
          <div className="space-y-1.5">
            ...
            <select 
              className="w-full bg-gray-50 border border-gray-100 px-3 py-2.5 rounded-xl text-[11px] sm:text-xs font-bold focus:bg-white focus:border-indigo-600 focus:ring-4 focus:ring-indigo-600/5 outline-none transition-all cursor-pointer appearance-none"
              value={filterYear}
              onChange={(e) => {
          ...
          <div className="flex flex-col justify-end col-span-2 md:col-span-1 pt-1 sm:pt-0">
             <button 
               onClick={() => { setFilterDate(""); setFilterMonth(""); setFilterYear(""); }}
               className="h-[46px] w-full bg-gray-100 text-gray-500 rounded-xl font-[900] text-[10px] uppercase tracking-widest hover:bg-gray-200 transition-all font-bold"
             >
               Clear Filters
             </button>
          </div>
                ...
                <div key={slot._id} className="bg-white border border-gray-100 p-5 rounded-[24px] shadow-sm flex items-center justify-between gap-4 hover:shadow-md transition-shadow">
                   <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-tight">Class {slot.classLevel} • {slot.mentor?.name}</p>
                      </div>
                   </div>
