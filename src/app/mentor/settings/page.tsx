...
export default function MentorSettingsPage() {
  ...
  useEffect(() => {
    api.get("/mentors/settings").then((res) => {
      setUser(res.data);
    }).catch(err => {
  ...
  const changePassword = async () => {
    ...
    try {
      await api.patch("/mentors/settings/password", {
        currentPassword,
        newPassword,
      });
  ...
  if (!user) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4">
        ...
        <p className="text-slate-400 font-[900] text-[10px] uppercase tracking-widest">Securing connection...</p>
      </div>
    );
  }
      ...
      <div className="max-w-5xl mx-auto p-3 sm:p-6 md:p-10 space-y-8 md:space-y-12 animate-fade-up">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:gap-10">
          ...
          <div className="group bg-white rounded-[32px] p-4 sm:p-8 shadow-sm border border-slate-100 hover:shadow-2xl hover:shadow-slate-200/50 transition-all duration-500 flex flex-col h-full">
            <div className="flex items-center gap-4 mb-8">
              <div className="w-12 h-12 rounded-2xl bg-indigo-50 flex items-center justify-center border border-indigo-100/50 shadow-sm text-indigo-600 transition-transform group-hover:scale-110 duration-500">
                <User className="w-6 h-6" />
              </div>
              <div>
                <h2 className="text-xl font-[900] text-slate-900 tracking-tight">Identity</h2>
                <p className="text-[10px] uppercase tracking-widest font-black text-slate-400">Public Information</p>
              </div>
            </div>

            <div className="space-y-5 flex-1">
              <div className="space-y-1.5 group/input">
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 transition-colors group-focus-within/input:text-primary">Legal Name</label>
              ...
              >
                {loading ? (
                  ...
                  <> 
                    ...
                    <span className="text-xs uppercase tracking-[0.2em]">Update Key</span>
                  </> 
                )}
              </button> 
            </div>
        ...
        <div className="text-center pt-8 border-t border-slate-50">
          <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest flex items-center justify-center gap-2">
            ...
            Data encrypted with 256-bit AES protocol
          </p>
        </div>
      </div>
    </div>
